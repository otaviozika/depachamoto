...
DO $$ BEGIN
  ALTER TABLE public.dispatches ADD CONSTRAINT dispatches_shift_code_check
...
ON public.dispatches(operational_date,shift_code,courier_id);
...
BEGIN
  IF TG_OP='UPDATE' AND OLD.shift_code IS NOT NULL THEN
    IF NEW.shift_code IS DISTINCT FROM OLD.shift_code OR NEW.operational_date IS DISTINCT FROM OLD.operational_date THEN
  ...
  ELSE derived_shift:=NULL; END IF;
  NEW.operational_date:=local_ts::date;
