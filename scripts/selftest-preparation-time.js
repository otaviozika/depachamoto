...
const checks = {
  version_350: server.includes('const VERSION = "3.7.0";') && pkg.version === '3.7.0' && html.includes('<title>DespacheFull 3.7.0</title>'),
  ...
  dynamic_sync: server.includes('queueIfoodPreparationTimeSync(merchantIds, reason)') && server.includes('reason: "preparation_time"'),
  ...
  pwa_cache: sw.includes('despachefull-v3.7.0-')
};

const failed = Object.entries(checks).filter(([, ok]) => !ok).map(([name]) => name);
if (failed.length) {
  console.error(JSON.stringify({ result: 'FAIL', version: '3.7.0', failed, checks }, null, 2));
  process.exit(1);
}
