...
const checks={
  version: pkg.version==='3.5.3' && server.includes('const VERSION = "3.5.3";'),
  ...
  pwa_cache: sw.includes('despachefull-v3.5.3-cancel-reason-codes')
};
const failed=Object.entries(checks).filter(([,v])=>!v).map(([k])=>k);
if(failed.length){console.error(JSON.stringify({result:'FAIL',failed,checks},null,2));process.exit(1)}
console.log(JSON.stringify({result:'PASS',version:'3.5.3',checks},null,2));
