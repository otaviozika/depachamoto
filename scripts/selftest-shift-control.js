...
const checks = {
  version_320: server.includes('const VERSION = "3.2.0";') && pkg.version === '3.2.0',
  ...
  pwa_cache_bumped: /despachefull-v3\.2\.0-shift/.test(sw)
};

for (const [name, ok] of Object.entries(checks)) assert.ok(ok, `FAIL: ${name}`);
console.log(JSON.stringify({ result: 'PASS', version: '3.2.0', checks }, null, 2));
