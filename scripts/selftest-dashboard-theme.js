...
const checks={
  version:server.includes('const VERSION = "3.3.0";')&&html.includes('<title>DespacheFull 3.3.0</title>'),
  darkAdmin:html.includes('/* v3.3.0 — interface administrativa sóbria */')&&html.includes('#adminApp .admin-nav button.active:before')&&html.includes('#adminApp .dashboard-metric-card:after{display:none!important}'),
  ...
  footer:html.includes('class="dashboard-footer"')&&html.includes('Versão 3.3.0'),
  wallboardPreserved:html.includes('PEDIDOS EM PREPARO')&&html.includes('id="kdsPreparingList"'),
  cache:sw.includes('despachefull-v3.3.0-sober-ui')
};
