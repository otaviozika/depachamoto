...
const loginIpBurstLimiter = rateLimit({
...
async function upsertAnotaAiOrder(pageId, normalized) {
  ...
  await pool.query(`
    ...
    ON CONFLICT(order_id) DO UPDATE SET
      ...
      display_id=CASE
        WHEN EXCLUDED.display_id IS NULL
          OR EXCLUDED.display_id=''
          OR LOWER(EXCLUDED.display_id)=LOWER(EXCLUDED.order_id)
          OR EXCLUDED.display_id ~* '^[a-f0-9]{24}
  `, [
    normalized.orderId,
    pageId,
...
function extractAnotaAiOrderCandidate(body, expectedOrderId = null) {
  ...
  while (queue.length) {
    ...
    if (humanReference !== null && humanReference !== undefined && String(humanReference).trim()) {
      const humanText = String(humanReference).replace(/^#/,"").trim();
      // O número visível do pedido vale mais que um wrapper que só repete o ObjectId técnico.
      score += /^[a-f0-9]{24}$/i.test(humanText) ? 5 : 220;
    }
...
function findNestedAnotaAiField(value, keys, depth = 0, seen = new Set()) {
  ...
  for (const child of children) {
    if (child && typeof child === "object") {
      const found = findNestedAnotaAiField(child, keys, depth + 1, seen);
    ...
    while (currentPage <= 20) {
      ...
      for (const summary of docs) {
        ...
        if (shouldFetch) {
          ...
          const nestedReference = findNestedAnotaAiField(detailsBody, [
            "shortReference","short_reference","displayId","display_id","orderNumber","order_number"
          ]);
          const nestedReferenceText = String(nestedReference || "").replace(/^#/,"").trim();
          if (nestedReferenceText &&
              nestedReferenceText.toLowerCase() !== String(summaryOrder.orderId).toLowerCase() &&
              !/^[a-f0-9]{24}$/i.test(nestedReferenceText)) {
            normalized.displayId = nestedReferenceText;
          }

          if (!normalized.orderType || String(normalized.orderType).toUpperCase() === "UNKNOWN") {
            const nestedOrderType = findNestedAnotaAiField(detailsBody, ["orderType","order_type"]);
            if (nestedOrderType) normalized.orderType = String(nestedOrderType).trim().toUpperCase();
          }
        }
...
async function ensureIfoodDispatchJob(orderId) {
  await pool.query(`
    ...
    WHERE l.ifood_order_id=$1
      AND l.ifood_dispatch_status <> 'DISPATCHED'
...
async function refreshIfoodOrderClassificationForDeparture(row) {
  ...
  const needsRefresh = orderType !== "DELIVERY" ||
...
async function inspectIfoodOrdersForDeparture(orders, { recovery = false, allowCompleted = false, orderId = null, referenceAt = new Date() } = {}) {
  ...
  for (const localOrder of orders) {
    ...
    if (!recovery && !ifoodOrderCanBeSelected(status)) {
      blocked.push({
        order_number: localOrder,
        order_id: row.order_id,
...
async function createDispatchTransaction({
  ...
  try {
    ...
    for (const link of ifoodLinks) {
      ...
      `, [link.order_id, dispatch.id, link.order_number, recovery ? "DISPATCHED" : "NOT_SENT"]);
      ...
      if (!recovery) await client.query(`
        INSERT INTO ifood_dispatch_jobs(
    ...
    if (append || recovery) await client.query(
    ...
    dispatch.order_numbers = (await client.query('SELECT order_number FROM dispatch_orders WHERE dispatch_id=$1 ORDER BY id', [dispatch.id])).rows.map(x=>x.order_number);
...
async function getDispatchProgressMap(dispatchIds, client = pool) {
...
async function markDispatchReturning(dispatchId, { actorUserId = null, source = "MANUAL", reason = null } = {}) {
  ...
  try {
    await client.query('BEGIN');
    // The same row is locked when appending: a completed-order event must not
    // switch the route to RETURNING after a new pending order was attached.
    await client.query('SELECT id FROM dispatches WHERE id=$1 FOR UPDATE', [dispatchId]);
    if (source.startsWith('AUTO_') || source === 'COURIER_MANUAL') {
      const progress = (await getDispatchProgressMap([dispatchId], client)).get(Number(dispatchId));
      const blocked = source.startsWith('AUTO_') ? !progress?.all_orders_resolved
        : !progress || progress.resolved_orders < progress.trackable_orders;
      if (blocked) {
        await client.query('ROLLBACK');
        if (source === 'COURIER_MANUAL') throw Object.assign(new Error('Confirme as entregas pendentes antes de iniciar o retorno.'), { status: 409 });
        return null;
      }
    }
...
async function getCourierDeliveryCount(courierId,date,shiftCode=null,client=pool) {
  const shift=['LUNCH','DINNER'].includes(String(shiftCode||'').trim().toUpperCase())?String(shiftCode).trim().toUpperCase():null;
...
function validateDepartureCount(body) {
  const count = body.order_count;
  const raw = body.order_numbers;
  if (!Number.isInteger(count) || count < 1 || count > 5 || !Array.isArray(raw) || raw.length !== count ||
      raw.some(x=>typeof x !== 'string' || !plainLocalOrderNumber(x)) ||
      new Set(raw.map(plainLocalOrderNumber)).size !== count) {
    throw Object.assign(new Error("Escolha de 1 a 5 pedidos e preencha exatamente essa quantidade, sem repetir."), { status: 400 });
  }
}
...
process.on("SIGINT", () => gracefulShutdown("SIGINT"));
        ...
        THEN anotaai_orders.display_id
        ELSE EXCLUDED.display_id
      END,
      status=EXCLUDED.status,
      status_code=EXCLUDED.status_code,
      sales_channel=CASE
        WHEN COALESCE(EXCLUDED.sales_channel,'')='' THEN anotaai_orders.sales_channel
        ELSE EXCLUDED.sales_channel
      END,
      order_type=CASE
        WHEN UPPER(COALESCE(EXCLUDED.order_type,'')) IN ('','UNKNOWN')
        THEN anotaai_orders.order_type
        ELSE EXCLUDED.order_type
      END,
      customer_name=COALESCE(NULLIF(EXCLUDED.customer_name,''),anotaai_orders.customer_name),
