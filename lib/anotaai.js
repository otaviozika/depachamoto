...
export function createAnotaAiClient({
  ...
  async function postOrderAction(pageId, orderId, action) {
    const id = String(orderId || "").trim();
    const operation = String(action || "").trim().toLowerCase();
    if (!id) throw new Error("ID do pedido Anota AI não informado.");
    if (!["ready","finalize"].includes(operation)) {
      throw new Error("Operação de status Anota AI inválida.");
    }

    const { body } = await authorizedFetch(
      `${ANOTAAI_ORDERS_BASE}/order/${operation}/${encodeURIComponent(id)}`,
      {
        method: "POST",
        headers: pageHeaders(pageId)
      }
    ...
    if (body?.success === false) {
      const error = new Error(String(body?.message || `O Anota AI recusou a operação ${operation}.`));
      error.anotaAiBody = safeBody(body);
      throw error;
    }
    return body;
  ...
  async function readyOrder(pageId, orderId) {
    return postOrderAction(pageId, orderId, "ready");
  }

  async function finalizeOrder(pageId, orderId) {
    return postOrderAction(pageId, orderId, "finalize");
  }
